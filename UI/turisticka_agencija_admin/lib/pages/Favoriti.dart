...
import 'package:turisticka_agencija_admin/providers/lista_zelja_provider.dart';
...
class _FavoritiState extends State<Favoriti> {
                  ...
                  children: [
                    Column(children: [
                      Image(
                          ...
                          height: 100,
                          image: MemoryImage(
                              const Base64Decoder().convert(lz.slika))),
  ...
  Future<List<Putovanja?>> getListaZelja() async {
    ...
    var listZeljaList =
        listaZelja!.map((i) => _listaZeljaProvider?.fromJson(i)).toList();

    var putovanja = await _putovanjaProvider?.get(null);
    var putovanjaList =
        putovanja?.map((i) => _putovanjaProvider?.fromJson(i)).toList();
